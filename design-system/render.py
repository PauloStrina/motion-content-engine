...
Uso: python render.py slides/episodio.json salida.pdf
...
# Regla fija: sombra ÚNICA por fondo — oscura sobre claro, clara sobre oscuro
...
def lam(word, size, front, shadow_hex, bg_hex, depth=10):
    """Degradé laminado monocolor: capas 1px del color de sombra fundiéndose hacia el fondo."""
    line = int(size*1.06); layers = ''
    for i in range(depth, 0, -1):
        col = mix(shadow_hex, bg_hex, i/depth*0.85)   # la capa más profunda casi se disuelve
...
def block_html(b, bg_hex):
    ...
    if t == 'rows':   # filas con opacidad decreciente (loop problema→solución)
        rows = ''
